...
superuser:
	docker-compose -f docker-compose.dev.yml run --rm django python manage.py createsuperuser --email admin@example.com --username admin

clean-db:
	set -a && source .env && docker-compose -f docker-compose.dev.yml down && docker volume rm --force $$DOCKER_DB_VOLUME
